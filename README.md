...
**[📋 更新日志](./CHANGELOG.md)** | **[🧪 基准测试](./TEST_BENCHMARKS.md)**
...
> - **FastVM**：优化的 VM（指令重排序、分支预测优化，推荐使用）
> - 比率 < 1 表示快于 CPython
> - GoPy 执行时间包含编译 + 解释两部分。查看详细基准测试结果请访问 [基准测试文档](./TEST_BENCHMARKS.md)。
...
| 优先级 | 优化项 | 预期收益 | 状态 |
|-------|-------|---------|------|
| P0 | 大整数对象池（复用 big.Int 对象） | 大整数操作加速 1.5-2x | 待启动 |
| P0 | 小整数范围快速路径（int64 直接操作） | 整数运算加速 | 待启动 |
| P1 | Bytecode Peephole 优化（编译时优化） | 指令序列优化 | 待启动 |
| P1 | 栈上分配策略（短期对象优化） | GC 压力降低 | 待启动 |

### 阶段三：长期优化（1-2月）

| 优先级 | 优化项 | 预期收益 | 状态 |
|-------|-------|---------|------|
| P1 | 分代垃圾回收（年轻代优先） | GC 停顿优化 | 规划中 |
| P1 | JIT 热点路径特化 | 热点代码加速 5-10x | 规划中 |
| P2 | 并行执行支持（协程安全 VM 池） | 多核利用 | 规划中 |
...
### 阶段一：高优先级（基于 Desugar 实现）

所有阶段一语法特性已完成，详细记录请参考 [CHANGELOG.md](./CHANGELOG.md)：

| 优先级 | 特性 | 实现方式 | 状态 |
|-------|------|---------|------|
| P0 | 位运算（`\|`, `&`, `^`, `~`, `<<`, `>>`） | 通过 desugar 转换为内置函数调用 | ✅ 已完成 |
| P0 | 身份运算符（`is`, `is not`） | 通过 desugar 转换为 id() 比较 | ✅ 已完成 |
| P0 | 成员运算符（`in`, `not in`） | 通过 desugar 转换为内置函数调用 | ✅ 已完成 |
| P0 | 元组解包（`a, b = 1, 2`） | 通过 desugar 展开为多个赋值语句 | ✅ 已完成 |

### 阶段二：中优先级

| 优先级 | 特性 | 说明 | 状态 |
|-------|------|------|------|
| P1 | 增强赋值（`\|=`, `&=`, `^=`, `<<=`, `>>=`） | 扩展现有增强赋值实现 | ✅ 已完成 |
| P1 | 生成器表达式（`(x for x in iter)`） | 通过 desugar 转换为生成器函数 | ✅ 已完成 |
| P1 | 多行字符串（`"""..."""`） | 词法分析器支持三引号字符串 | ✅ 已完成 |

### 阶段三：低优先级

| 优先级 | 特性 | 说明 | 状态 |
|-------|------|------|------|
| P2 | 装饰器（`@decorator`） | 需要修改 AST 和编译器 | 规划中 |
| P2 | 异步编程（`async def`, `await`） | 需要协程支持 | 规划中 |
| P2 | 类型注解（`def f(x: int) -> str`） | 需要类型系统支持 | 规划中 |
