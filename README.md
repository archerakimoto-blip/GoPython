...
| 测试用例 | GoPy | GoPy+JIT | FastVM | FastVM+JIT | CPython | 比率 G/CP | 比率 F/CP | 比率 FJ/CP |
|---------|------|----------|--------|------------|---------|-----------|-----------|------------|
| 简单计算 | 452.2 ms | 422.5 ms | 309.1 ms | 507.9 ms | 54.8 ms | 8.25x | 5.64x | 9.26x |
| 函数调用 | 450.4 ms | 570.3 ms | 419.9 ms | 511.5 ms | 77.7 ms | 5.79x | 5.40x | 6.58x |
| 循环操作 | 563.9 ms | 438.4 ms | 479.5 ms | 425.3 ms | 74.5 ms | 7.56x | 6.43x | 5.70x |
| **平均** | **488.8 ms** | **477.1 ms** | **402.8 ms** | **481.5 ms** | **69.0 ms** | **7.08x** | **5.83x** | **6.97x** |

> **说明**：
> - **GoPy**：标准 VM（调试代码已移除）
> - **GoPy+JIT**：标准 VM + JIT 编译器
> - **FastVM**：优化的 VM 解释器（最快）
