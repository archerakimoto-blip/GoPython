...
⚠️ 缺失 eval, exec, compile, memoryview, __import__, breakpoint
...
- [x] **B1.1**: `str.find()` — 查找子串位置，支持 start/end 参数
- [x] **B1.2**: `str.index()` — 查找子串位置，不存在抛 ValueError
- [x] **B1.3**: `str.replace()` — 子串替换，支持 count 参数
- [x] **B1.4**: `str.split()` — 字符串分割，支持 sep/maxsplit 参数
- [x] **B1.5**: `str.rsplit()` — 右分割
- [x] **B1.6**: `str.splitlines()` — 按行分割
- [x] **B1.7**: `str.join()` — 连接序列
- [x] **B1.8**: `str.strip()` / `str.lstrip()` / `str.rstrip()` — 去除首尾字符
- [x] **B1.9**: `str.upper()` / `str.lower()` — 大小写转换
- [x] **B1.10**: `str.startswith()` / `str.endswith()` — 前缀/后缀判断
- [x] **B1.11**: `str.format()` — 格式化字符串
- [x] **B1.12**: `str.casefold()` — 大小写折叠
- [x] **B1.13**: `str.maketrans()` — 创建转换表（实例方法版）

#### B2. 列表方法补齐（优先级：高）

- [x] **B2.1**: `list.append()` — 追加元素
- [x] **B2.2**: `list.extend()` — 扩展列表
- [x] **B2.3**: `list.insert()` — 插入元素
- [x] **B2.4**: `list.remove()` — 删除首个匹配元素
- [x] **B2.5**: `list.pop()` — 弹出指定位置元素
- [x] **B2.6**: `list.clear()` — 清空列表
- [x] **B2.7**: `list.index()` — 查找元素索引
- [x] **B2.8**: `list.count()` — 统计元素出现次数
- [x] **B2.9**: `list.reverse()` — 反转列表
- [x] **B2.10**: `list.copy()` — 浅拷贝

#### B3. 集合方法补齐（优先级：中）

- [x] **B3.1**: `set.add()` — 添加元素
- [x] **B3.2**: `set.remove()` — 删除元素（不存在抛 KeyError）
- [x] **B3.3**: `set.discard()` — 安全删除元素
- [x] **B3.4**: `set.pop()` — 弹出元素
- [x] **B3.5**: `set.clear()` — 清空集合
- [x] **B3.6**: `set.intersection_update()` — 交集更新
- [x] **B3.7**: `set.symmetric_difference_update()` — 对称差更新

#### B4. 元组/整数/浮点/字节方法补齐（优先级：中）

- [x] **B4.1**: `tuple.count()` / `tuple.index()` — 元组方法
- [x] **B4.2**: `int.bit_length()` / `int.bit_count()` / `int.to_bytes()` — 整数方法
- [x] **B4.3**: `float.is_integer()` / `float.hex()` / `float.as_integer_ratio()` — 浮点方法
- [x] **B4.4**: `bytes.decode()` / `bytes.hex()` — 字节方法

#### B5. 缺失内置函数补齐（优先级：高）

- [x] **B5.1**: `divmod()` — 商和余数
- [x] **B5.2**: `frozenset()` — 冻结集合构造
- [x] **B5.3**: `pow()` 三参数版 — 幂运算 + 取模（待完善）
- [x] **B5.4**: `delattr()` — 删除属性
- [x] **B5.5**: `globals()` / `locals()` — 返回变量字典（简化实现）
- [x] **B5.6**: `vars()` — 返回对象 __dict__
- [ ] **B5.7**: `eval()` / `exec()` — 动态执行代码（需 VM 集成）
- [ ] **B5.8**: `compile()` — 编译源代码（需 VM 集成）
- [x] **B5.9**: `bytearray()` — 字节数组构造
- [x] **B5.10**: `bytes()` — 字节构造
- [ ] **B5.11**: `memoryview()` — 内存视图（需新类型）
- [x] **B5.12**: `slice()` — 切片对象构造
- [x] **B5.13**: `ascii()` — 返回 ASCII 表示
- [x] **B5.14**: `object()` — 基础对象构造
- [ ] **B5.15**: `__import__()` — 模块导入函数（需 VM 集成）
- [ ] **B5.16**: `breakpoint()` — 调试断点（需调试器集成）

#### B6. 缺失异常类型补齐（优先级：中）

- [x] **B6.1**: `Exception` / `BaseException` — 基础异常类
- [x] **B6.2**: `AssertionError` — 断言错误
- [x] **B6.3**: `OSError` / `IOError` / `FileNotFoundError` / `FileExistsError` / `PermissionError` — 系统异常族
- [x] **B6.4**: `ImportError` / `ModuleNotFoundError` — 导入异常
- [x] **B6.5**: `SyntaxError` / `IndentationError` — 语法异常
- [x] **B6.6**: `UnicodeError` / `UnicodeDecodeError` / `UnicodeEncodeError` — Unicode 异常
- [x] **B6.7**: `RecursionError` — 递归错误
- [x] **B6.8**: `IsADirectoryError` / `NotADirectoryError` — 目录异常
- [x] **B6.9**: `BufferError` / `ArithmeticError` / `LookupError` / `ReferenceError` / `StopAsyncIteration` — 其他异常

#### B7. 标准库模块补齐（优先级：中）

- [x] **B7.1**: math — `atan2, copysign, fmod, isnan, isinf, isfinite, factorial, gcd, lcm, inf, nan, tau`
- [x] **B7.2**: os — `rmdir, makedirs, removedirs, stat, os.name, os.linesep, os.curdir, os.pardir`；os.path 子模块 — `exists, isfile, isdir, join, split, basename, dirname, getsize, abspath`
- [x] **B7.3**: json — `dump, load`
- [x] **B7.4**: random — `randrange, sample, choices, gauss`
- [x] **B7.5**: time — `strftime, strptime, gmtime, mktime, time_ns, monotonic, perf_counter, timezone, tzname`
- [ ] **B7.6**: datetime — 重构为对象返回（非字符串），补齐 `timedelta, strptime, fromtimestamp, year/month/day 等属性访问`（待完善）
- [x] **B7.7**: sys — `maxsize, byteorder, executable, prefix, modules, flags`

#### B8. 缺失类型补齐（优先级：低）

- [x] **B8.1**: `bytearray` 类型 — 简化实现（复用 Bytes）
- [x] **B8.2**: `frozenset` 类型 — 不可变集合（含 union/intersection/issubset/issuperset/copy）
- [ ] **B8.3**: `memoryview` 类型 — 内存视图（待实现）
- [ ] **B8.4**: `slice` 对象 — 切片对象类型（待实现）
