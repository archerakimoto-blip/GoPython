...
| # | 特性 | 当前状态 | 影响 |
|---|------|----------|------|
| P0-1 | Raw strings `r"..."` | ✅ 已实现 RSTRING token | 正则表达式、Windows 路径可用 |
| P0-2 | Byte strings `b"..."` | 无 BSTRING token | 字节操作不可用 |
| P0-3 | 单引号字符串 `'...'` | ✅ 已实现 | Python 最常用写法可用 |
| P0-4 | 三引号字符串 `"""..."""` | ✅ 已实现 | 文档字符串、多行文本可用 |
| P0-5 | 位运算符 `& \| ^ ~ << >>` | ✅ 已实现 token + 脱糖 | 位运算可用 |
| P0-6 | 位运算增强赋值 `&= \|= ^= <<= >>=` | ✅ 已实现 token | 位运算增强赋值可用 |
| P0-7 | `is` / `is not` 运算符 | ✅ 已实现 IS token + 脱糖 | 身份比较可用 |
| P0-8 | `in` / `not in` 成员运算符 | ✅ 已实现 IN token + 脱糖 | 成员测试可用 |
| P0-9 | `assert` 语句 | ✅ 已实现 ASSERT 关键字 + 脱糖 | 断言可用 |
...
- [x] P1-14：异常链 `raise E from e` 脱糖
- [x] P1-15：多异常类型 `except (A, B)` 脱糖
...
- [x] P1-8：`@dataclass` 装饰器脱糖
- [x] P1-11：默认参数值 — 修改解析器存储默认值 + 脱糖层处理
- [x] P1-7：`super()` 脱糖
- [ ] P1-10：`@functools.lru_cache` 脱糖
- [x] P1-16：f-string 格式化规格脱糖
- [x] P1-17：多 for 子句推导式脱糖
- [x] P0-4：三引号字符串
- [x] P0-5：位运算符 token + P1-4 位运算脱糖
...
- [x] P3-5：特化操作码 — VM 内联快速路径（int+int 直接计算）
- [x] P3-13：常量折叠 — 脱糖层编译期计算常量表达式
- [ ] P3-12：对象池
