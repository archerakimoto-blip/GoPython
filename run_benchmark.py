...
def main():
    ...
    print("| Test Case            | GoPy | GoPy+JIT | FastVM | CPython | Ratio G/CP | Ratio F/CP |")
    print("|---------------------|------|----------|--------|---------|-----------|-----------|")
    ...
    for file_path, name in tests:
        ...
        try:
            ...
            print(f"| {name:19} | {gopy_ms:5.1f}ms | {gopy_jit_ms:8.1f}ms | {fastvm_ms:7.1f}ms | {cpython_ms:7.1f}ms | {ratio_g:9.2f}x | {ratio_f:9.2f}x |")
    ...
    if count > 0:
        ...
        print("|---------------------|------|----------|--------|---------|-----------|-----------|")
        print(f"| **Average**         | **{avg_gopy:4.1f}ms** | **{avg_gopy_jit:7.1f}ms** | **{avg_fastvm:6.1f}ms** | **{avg_cpython:6.1f}ms** | **{avg_ratio_g:9.2f}x** | **{avg_ratio_f:9.2f}x** |")

    print()
    print("=" * 120)
    print("Notes:")
    print("- GoPy: Standard VM")
    print("- GoPy+JIT: Standard VM with JIT compilation (framework only)")
    print("- FastVM: Optimized VM with instruction reordering")
